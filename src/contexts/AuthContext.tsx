...
interface AuthContextType {
  ...
  signUp: (email: string, password: string, fullName: string) => Promise<{ error: Error | null; needsConfirmation: boolean; userId?: string }>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Live-sync own profile changes (avatar, name, etc.)
  useEffect(() => {
    if (!user) return;
  ...
  const signUp = async (email: string, password: string, fullName: string) => {
    ...
    return { error: error as Error | null, needsConfirmation, userId: data.user?.id };
  };
